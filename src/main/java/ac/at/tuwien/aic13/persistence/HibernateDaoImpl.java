...
import ac.at.tuwien.aic13.persistence.exception.DaoException;
...
public class HibernateDaoImpl implements GenericDao {
	...
	private static final int MAX_RESULTS_PER_LIST = 100;
	...
	public <T extends DTO> T read(T element) {
		T ret = (T) sessionFactory.getCurrentSession().get(element.getClass(), element);				logger.info("retrieved " + ret.toString());
		return ret;
	}
	...
	public <T extends Serializable> List<T> findByNamedQueryAndNamedParams(Class<T> clazz, String hql,	Map<String, ? extends Object> params, Map<String, ? extends Collection<? extends Object>> parameterLists) throws DaoException {
		return findByNamedQueryAndNamedParams(clazz, hql, 500, params, parameterLists);
	}
	...
	public <T extends DTO> List<T> findByDetachedCriteria(DetachedCriteria dc, int lazyLoadLevel)
		...
		c.setMaxResults(MAX_RESULTS_PER_LIST);
	...
	public <T extends DTO> boolean exists(T element) throws DaoException{
		try{
		Query query = sessionFactory.getCurrentSession().createQuery("select 1 from "+ element.getClass().getSimpleName() + " t where t = :key");  
		query.setEntity("key", element);
